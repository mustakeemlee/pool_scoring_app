...
describe('CorrectMatchPage', () => {
  ...
  it('submits a valid correction, shows a success toast, invalidates the dependent caches, and returns to the list', async () => {
    ...
    // match results, in this order, so the open-matches list, leaderboard, match history, and
    // both players' profiles refresh without a manual reload. Asserting exact call count plus
    // each call's position (not just membership) means this test fails if an invalidation is
